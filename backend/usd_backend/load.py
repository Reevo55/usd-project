...
def create_teacher(teacher, access_token):
    payload = {"name": teacher[1], "title": teacher[0]}

    response = requests.post(
        TEACHERS_URL, data=payload, headers={"Authorization": f"Bearer {access_token}"}
    )
    json_data = json.loads(response.text)
    print(json_data)
    return json_data["id"]
...
def get_teacher_id(teacher_name, access_token):
    response = requests.get(
        TEACHERS_URL, headers={"Authorization": f"Bearer {access_token}"}
    )
    json_data = json.loads(response.text)
...
def get_course_id(course_data, access_token):
    response = requests.get(
        COURSES_URL, headers={"Authorization": f"Bearer {access_token}"}
    )
    json_data = json.loads(response.text)
...
def create_course(course, access_token):
    if course["teacher_id"] != "":
        ...
        teacher = [" ".join(teacher[:-2]), " ".join(teacher[-2:])]
    ...
    response = requests.post(
        COURSES_URL, data=payload, headers={"Authorization": f"Bearer {access_token}"}
    )
    json_data = json.loads(response.text)
...
def get_lesson_id(lesson_data, course_id, access_token):
    response = requests.get(
        f"{COURSES_URL}{course_id}{LESSONS_URL}",
...
def main():
    ...
    for course in courses:
        course_id = get_course_id(
            {"name": course["name"], "lesson_type": course["lesson_type"]}, access_token
        )
