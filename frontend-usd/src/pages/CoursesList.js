...
const mapCourses = (courses) => (
    courses.map(({ code, time, weekDay, teacher, lessonType, link }) =>
        <div className="course-list-item">
            ...
            <Text className="longer">{teacher.title} {teacher.teacherName}</Text>
            <Text className="long" style={{ fontWeight: '600' }}>{lessonType}</Text>
            <Text><Link to={link}>link</Link></Text>
        </div>
    )
);
