...
const Sidebar = () => {
    ...
    const mapMenu = (items) => {
        return (
            items.map(({ title, path, icon }) =>
                <Menu.Item key={title} icon={icon}><Link to={path}>{title}</Link></Menu.Item>
            )
        );
    }
    ...
    const mapCourses = (items) => {
        return (
            items.map(({ courseName, tag }) =>
                <Menu.Item key={courseName}><Link to='/courses' className="menu-item-hashtag">#{courseName.length < 24 ? courseName : tag}</Link></Menu.Item>
            )
        );
    }

    return (
        <Sider collapsible collapsed={isCollapsed} collapsedWidth={80}
            theme="light" width={250} className="layout-menu"
